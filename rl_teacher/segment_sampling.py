...
def create_segment_q_states(segment):
    obs_Ds = segment["obs"]
    try:
        act_Ds = segment["actions"].reshape(-1,1)
        cat = np.concatenate([obs_Ds, act_Ds], axis=1)
    except:
...
def do_rollout(env, action_function):
    ...
    for i in range(max_timesteps_per_episode):
        ...
        try:
            ...
            ob, rew, done, info = env.unwrapped.step(action)
        rewards.append(rew)
...
def segments_from_rand_rollout(env_id,
                            make_env,
                            n_desired_segments,
    ...
    if workers is None: #maximize the number of workers
        pool = Pool()
    else:
