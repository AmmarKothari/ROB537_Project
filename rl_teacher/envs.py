...
import gym
import numpy as np
from gym.envs import mujoco, classic_control
...
class MjViewer(TransparentWrapper):
    ...
    def __init__(self, env, fps=40):
        self.fps = fps
        super(TransparentWrapper, self).__init__(env)
    ...
    def render_full_obs(self, full_obs):
        ...
        self.env.render()
        data, width, height = self.env._get_viewer().get_image()
...
def acrobot(short=False):
    env = classic_control.AcrobotEnv()
    env = UseReward(env, reward_info_key="reward_dist")
    env = MjViewer(fps=10, env=env)
    return limit(t=20 if short else 50, env=env)
