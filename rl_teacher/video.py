...
class SegmentVideoRecorder(object):
    ...
    def path_callback(self, path):
        if self._num_paths_seen % self.checkpoint_interval == 0:  # and self._num_paths_seen != 0:
            ...
            if self.save_paths:
                fname = '%s/run_%s_%s.csv' % (self.save_dir, self._num_paths_seen, self._counter)
                print("Saving Path of run %s_%s to %s" % (self._num_paths_seen, self._counter, fname))
                write_paths(path, fname)
...
def write_paths(path, fname):
    os.makedirs(osp.dirname(fname), exist_ok=True)
    with open(fname, "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        keys = list(path.keys())
        header_row = copy.deepcopy(keys)
        header_row.insert(0,'timestep')
        writer.writerow(header_row)
        for i in range(len(path[keys[0]])):
            row = [i]
            for k in keys:
                row.append(path[k][i])
            writer.writerow(row)
    ...
    with open(fname, 'r', newline="") as csvfile:
        ...
        for row in reader:
            for i,h in enumerate(header):
                if i == 0:
                    continue #just the time stamp
                if h == 'human_obs': #for dealing with arrays with more than one row
                    row_update = [r.replace('[','').replace(']','') for r in row[i].split('\n')]
                    path[h].append(np.array([r.split() for r in row_update]).astype(float))
                else:
                    path[h].append(np.array(row[i].strip('[]').split()).astype(float))
    for k in path.keys():
